...
public class RecordDao {
	...
	public ArrayList<CostModel> findByA(int category_id,String note){
		...
		try {
			conn = DBUtil.getConnection();
			String sql = "select r.id as id,m.`name` as member_name ,c.name as category_name ,sum,type_id,`date` from record as r left join category as c on c.id = r.category_id left join member as m on m.id = r.member_id where c.id =? or member_name like '%?%'";
			st = conn.prepareStatement(sql);
